...
class Day_7_Tests extends FunSpec {
    ...
    val testData:String =
        """|jlbcwrl (93)
          ...
          |hpuyku (82)""".stripMargin

    // Exercise 1
    describe("parseBranch(line:String): Branch[Int]") {
        it("should create the an object with the correct values ") {
            val branch: Branch[Int] = parseBranch("fwft (72) -> ktlj, cntj, xhth")

            assert(branch.name == "fwft")
            assert(branch.value == 72)
            assert(branch.nodeNames == List("ktlj", "cntj", "xhth"))
            assert(branch.nodes == Nil)
        }
    }

    describe("parseLeaf(line: String): Leaf[Int]") {
        it("should create the an object with the correct values ") {
            val leaf: Leaf[Int] = parseLeaf("pbga (66)")

            assert(leaf.name == "pbga")
            assert(leaf.value == 66)
        }
    }

    describe("getBranchesAndLeafs(data: String): ListOfNodes") {
        it("should return a list of separated nodes") {
            val listOfNodes = getBranchesAndLeaves(exampleData)

            assert(listOfNodes.branches.size == 4)
            assert(listOfNodes.leaves.size == 9)
        }
    }

    describe("getMapOfChildrenToParents(listOfBranches: List[Branch[Int]], acc: Map[String, String]) : Map[String, String]") {
        it("should return the list of child to parent relationships") {
            val listOfNodes = getBranchesAndLeaves(exampleData)

            val mapOfChildrenToParents = getMapOfChildrenToParents(listOfNodes.branches)

            assert(mapOfChildrenToParents.size == 12)
        }
    }

    describe("getRootNode(childToParentMap: Map[String, String]) : String") {
        it("should return the root node according to the map") {
            val listOfNodes = getBranchesAndLeaves(exampleData)

            val mapOfChildrenToParents = getMapOfChildrenToParents(listOfNodes.branches)

            val rootNodeName = getRootNode(mapOfChildrenToParents)

            assert(rootNodeName == "tknk")
        }
    }

    describe("getRootNodeNameFromData(data: String): String") {
        it("should return the root node name") {
            val rootNodeNameForExampleData = getRootNodeNameFromData(exampleData)

            assert(rootNodeNameForExampleData == "tknk")

            val rootNodeNameForTestData = getRootNodeNameFromData(testData)

            assert(rootNodeNameForTestData == "gynfwly")
        }
    }

    // Exercise 2
    describe("addLeavesToBranches(mapOfBranches: Map[String, Branch[Int]], listOfLeaves: List[Leaf[Int]], mapOfChildrenToParents: Map[String, String]): Map[String, Branch[Int]]") {
        it("should return the branches with the leaves added") {
            val rootNodeForTestData = constructTreeFromData(testData)

            assert(rootNodeForTestData.name == "gynfwly")
        }
    }

    describe("getSumOfNode(node: Node[Int]): Int") {
        it("should return the sum of a branch from a tree") {
            val rootNodeForExampleData: Node[Int] = constructTreeFromData(exampleData)

            assert(getSumOfNode(rootNodeForExampleData) == 778)
        }
    }

    describe("getValueThatUnbalancedNodeShouldBe(data: String): Option[Exercise2Result]") {
        it("should return the correct unbalanced value and the name of the unbalanced branch for the tree") {
            val testResult: Option[Exercise2Result] = getValueThatUnbalancedNodeShouldBe(testData)

            testResult match {
                case None => assert(false)
                case Some(result) => {
                    assert(result.correctValue == 1526)
                    assert(result.nodeName == "ycbgx")
                }
            }
        }
    }
