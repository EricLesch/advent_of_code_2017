...
class Day_8_Tests extends FunSpec {
    val exampleData =
        """b inc 5 if a > 1
          |a inc 1 if b < 5
          |c dec -10 if a >= 1
    ...
    val testData =
        """sdj dec 558 if r <= 8
          ...
          |dtf inc 304 if nwg > -1974""".stripMargin

    describe("getAllRegisters(data: String): List[String] ") {
        it("should be a return all of the registers in the program") {
            assert(getAllRegisters(exampleData) == List("b", "a", "c"))

            assert(getAllRegisters(testData) == List("sdj", "r", "cpv", "csu", "ke", "pep", "bs", "nj", "hp", "rcb", "t", "moe", "vt", "n", "dtf", "u", "zx", "nwg", "ib", "ifd", "pmp", "yk", "ojk", "s", "vdv", "if"))
        }
    }

    describe("getLargestValueInAnyRegisterForData(data:String): Int") {
        it("should return the greatest value in any register") {
            assert(getLargestValueInAnyRegisterForData(exampleData) == 1)

            assert(getLargestValueInAnyRegisterForData(testData) == 3880)
        }
    }

    describe("getLargestValueAtAnyPoint(data: String): Int ") {
        it("should return the greatest value at any point in the calculation") {
            assert(getLargestValueAtAnyPoint(exampleData) == 10)

            assert(getLargestValueAtAnyPoint(testData) == 5035)
        }
    }
}
